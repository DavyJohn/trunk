 ...
 */
public class K33BuTongDanActivity extends BaseActivity {
    private static final String TAG = "K33BuTongDanActivity";
    @Bind(R.id.tv_time)
    TextView tvTime;
    ...
    private void clickRedBall(TextView view, String tag) {
        if (view.getTag()  == null || view.getTag() == "0") {
            if (checkRedNum(tag)) {
                ...
                String toast = mRedList1.size() > 1 ? "胆码区最多只能选2个" : "拖码区最多只能选5个";
                showToast(toast);
            }
        } else {
            // 取消
    ...
    private boolean checkRedNum(String tag) {
        return tag.equals("1") ? !(mRedList1.size() > 1) : !(mRedList2.size() > 4);
    }
